...
function ProfileImage({ className = "" }: { className?: string }) {
  ...
  if (hasError) {
    return (
      <img 
        src="https://ui-avatars.com/api/?name=Jagadeesh+Arigala&size=256&background=f1f5f9&color=334155" 
        alt="Jagadeesh Arigala" 
        className={className} 
      />
    );
  }
...
function getLocalPortfolioReply(query: string): { intent: ChatMessage['type']; ai_text: string } {
  ...
  if (q.includes('cgpa') || q.includes('gpa') || q.includes('marks') || q.includes('percentage') || q.includes('score') || q.includes('rank')) {
    return {
      intent: 'general',
  ...
  if (q.includes('plant brain') || q.includes('hybrid graph')) {
    return {
      intent: 'general',
  ...
  if (q.includes('diabetic') || q.includes('retinopathy') || q.includes('fundus')) {
    return {
      intent: 'general',
  ...
  if (q.includes('skin') || q.includes('dermoscopy')) {
    return {
      intent: 'general',
  ...
  if (q.includes('whatsapp') || q.includes('booking')) {
    return {
      intent: 'general',
  ...
  // General Education
  if (q.includes('education') || q.includes('university') || q.includes('study') || q.includes('college') || q.includes('school') || q.includes('vit') || q.includes('narayana')) {
    return {
      intent: 'general',
  ...
  if (q.includes('projects') || q.includes('show projects') || q.includes('list projects') || q.includes('view projects') || q.includes('portfolio')) {
    return {
      intent: 'projects',
  ...
  // Skills trigger
  if (q.includes('skill') || q.includes('stack') || q.includes('technology') || q.includes('python') || q.includes('tensorflow') || q.includes('pytorch') || q.includes('certif')) {
    return {
      intent: 'skills',
  ...
  if (q.includes('experience') || q.includes('resume') || q.includes('work') || q.includes('job') || q.includes('ethara') || q.includes('teachnook') || q.includes('infosys')) {
    return {
      intent: 'resume',
      ai_text: "I have displayed Jagadeesh's professional experience timeline above, covering his roles at Ethara AI (Data Annotation & LLM Evaluator), Infosys Springboard (AI & ML Intern), and Teachnook (ML Intern)."
    };
  }

  // Contact trigger
  if (q.includes('contact') || q.includes('email') || q.includes('linkedin') || q.includes('github') || q.includes('phone') || q.includes('reach')) {
    return {
      intent: 'contact',
  ...
  if (q.includes('about yourself') || q.includes('who are you') || q.includes('tell me about yourself') || q.includes('who is jagadeesh')) {
    return {
      intent: 'me',
        ...
        <div className="flex-1">
          <AnimatePresence>
            {viewState === 'landing' && (
              <motion.a 
                initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                href="https://drive.google.com/file/d/1JwlzxLJXgMLPV_gIBeMIQLTUKcLJdToq/view?usp=sharing" 
                target="_blank" 
...
function useTypewriterRoles(
  ...
  useEffect(() => {
    const currentRole = rolesRef.current[roleIndex] || '';
    let timer: any;
  ...
  }, [displayText, isDeleting, roleIndex, typingSpeed, deletingSpeed, pauseDuration]);
...
function LandingView({ onQuery, theme }: { onQuery: (q: string, t: ChatMessage['type']) => void, theme?: string }) {
  ...
  const animatedRole = useTypewriterRoles(PORTFOLIO_ROLES);

  const dynamicPlaceholder = useTypewriterPlaceholder([
    "Ask me anything...",
    "Ask me about Plant Brain & Hybrid Graph-RAG...",
    "What is Jagadeesh's AI stack?",
    "Tell me about your Infosys Springboard internship...",
    ...
    <>
      ...
      >
        <div className="text-center mb-6">
          <h2 className={`text-2xl sm:text-3xl font-sans font-semibold tracking-wide mb-3 transition-colors duration-500 ${theme === 'dark' ? 'text-zinc-300' : 'text-slate-700'}`}>Hey, I'm Jagadeesh Arigala</h2>
          <h1 className={`text-4xl sm:text-6xl md:text-7xl font-serif font-bold tracking-tight min-h-[1.25em] flex items-center justify-center transition-colors duration-500 ${theme === 'dark' ? 'text-zinc-100' : 'text-slate-900'}`}>
            <span>{animatedRole}</span>
            <span className="animate-pulse text-blue-500 ml-1 font-mono">|</span>
          </h1>
        </div>
  ...
  return (
    <div className="w-full flex flex-col md:flex-row items-center md:items-start gap-8 mb-8">
      ...
      <div className="flex-1 flex flex-col text-left">
        <h1 className={`text-4xl font-bold mb-2 transition-colors duration-500 ${theme === 'dark' ? 'text-zinc-100' : 'text-slate-900'}`}>Jagadeesh Arigala</h1>
        <p className={`mb-4 font-medium transition-colors duration-500 ${theme === 'dark' ? 'text-zinc-400' : 'text-slate-505'}`}>AI Engineer | Software Engineer | Data Scientist <span className="mx-2">-</span> Bangalore, India</p>
        
        <p className={`leading-relaxed mb-6 transition-colors duration-500 ${theme === 'dark' ? 'text-zinc-300' : 'text-slate-700'}`}>
          BTech CSE (AI & ML) student at VIT-AP University with hands-on experience across the full ML stack — model training, LLM integration, API development, and cloud deployment. Building intelligent systems from computer vision pipelines to production-grade RAG and multi-agent platforms.
        </p>

        <div className="flex flex-wrap gap-2">
          <span className="bg-blue-600 text-white px-4 py-1.5 rounded-full text-[13px] font-normal shadow-sm">Hybrid Graph-RAG</span>
          <span className="bg-blue-600 text-white px-4 py-1.5 rounded-full text-[13px] font-normal shadow-sm">Computer Vision</span>
          <span className="bg-blue-600 text-white px-4 py-1.5 rounded-full text-[13px] font-normal shadow-sm">Python & TensorFlow</span>
          <span className="bg-blue-600 text-white px-4 py-1.5 rounded-full text-[13px] font-normal shadow-sm">FastAPI & Docker</span>
        </div>
      </div>
    </div>
  );
...
function ContactCard({ theme }: { theme?: string }) {
  const socials = [
    { name: 'LinkedIn', url: 'https://linkedin.com/in/jagadeesharigala' },
    { name: 'GitHub', url: 'https://github.com/Jagadeesh2205' },
    { name: 'Email', url: 'mailto:arigalajagadeesh@gmail.com' }
  ];
  ...
  return (
    <div className={`w-full border rounded-3xl p-6 md:p-8 mb-6 shadow-sm flex flex-col items-stretch text-left transition-colors duration-500 ${
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <span className="text-sm md:text-base text-slate-500 font-medium">@Jagadeesh2205</span>
      </div>
      
      <hr className={`mb-6 transition-colors duration-500 ${theme === 'dark' ? 'border-zinc-800/60' : 'border-slate-200/80'}`} />
      
      <a 
        href="mailto:arigalajagadeesh@gmail.com" 
        className="text-blue-500 hover:text-blue-600 font-normal text-lg md:text-xl mb-2 self-start transition-colors"
      >
        arigalajagadeesh@gmail.com
      </a>
      <p className="text-slate-500 dark:text-zinc-400 text-sm mb-6">Phone: +91 77805 88637 | Location: Bangalore, India</p>
